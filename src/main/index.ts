...
app.whenReady().then(() => {
  ...
  const teamsClientId = settingsStore.get('teamsClientId') || ''
  if (teamsClientId) {
    teamsService.setClientId(teamsClientId)
  }

  // Gespeicherte Konten aus SettingsStore laden
  const savedAccounts = settingsStore.get('teamsAccounts') || []
  if (savedAccounts.length > 0) {
    teamsService.loadAccounts(
      savedAccounts.map((a) => ({
        ...a,
        accessToken: '',
        refreshToken: '',
  ...
  // Token Persistenz: accountAdded / tokenRefreshed → in SettingsStore speichern
  teamsService.on('accountAdded', (account) => {
    const current = settingsStore.get('teamsAccounts') || []
    const existing = current.findIndex((a) => a.id === account.id)
    const accountInfo = {
      id: account.id,
    ...
    if (existing >= 0) {
      ...
      updated[existing] = accountInfo
      settingsStore.set('teamsAccounts', updated)
    } else {
      settingsStore.set('teamsAccounts', [...current, accountInfo])
    }
  })

  teamsService.on('tokenRefreshed', (account) => {
    const current = settingsStore.get('teamsAccounts') || []
    const updated = current.map((a) =>
      a.id === account.id
        ? { id: account.id, displayName: account.displayName, email: account.email, tenantId: account.tenantId }
        : a
    )
    settingsStore.set('teamsAccounts', updated)
  })
  ...
  teamsService.on('accountRemoved', (accountId: string) => {
    const current = settingsStore.get('teamsAccounts') || []
    settingsStore.set('teamsAccounts', current.filter((a) => a.id !== accountId))
  })
