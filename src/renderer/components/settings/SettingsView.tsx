...
export default function SettingsView() {
  ...
  // Teams hook
  const { accounts: teamsAccounts, deviceCode, login: teamsLogin, logout: teamsLogout } = useTeamsSync()
      ...
      <SectionCard title="Microsoft Teams" icon={<Wifi size={16} />}>
        <div className="flex flex-col gap-3">
          <ToggleRow
            ...
            onChange={(v) => {
              setTeamsEnabled(v)
              window.swyxApi?.teamsSetEnabled(v).catch(() => {})
            }}
          />
          ...
          {teamsEnabled && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    window.swyxApi?.teamsSetClientId(val).catch(() => {})
                  }}
                  onKeyDown={(e) => { if (e.key === 'Enter') e.currentTarget.blur() }}
                  className="w-full text-xs px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 font-mono transition-colors"
                />
                <p className="text-[11px] text-zinc-400 dark:text-zinc-500 leading-relaxed">
                  Aus der Azure AD App-Registrierung (portal.azure.com)
                </p>
              </div>

              {/* Device Code Flow (inline) */}
              {deviceCode && (
                <div className="rounded-lg bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 px-4 py-3 flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <Loader2 size={14} className="text-indigo-500 animate-spin flex-shrink-0" />
                    <span className="text-xs font-semibold text-indigo-700 dark:text-indigo-300">Warte auf Anmeldung...</span>
                  </div>
                  <p className="text-xs text-indigo-600 dark:text-indigo-400 leading-relaxed">
                    Öffnen Sie{' '}
                    <button
                      onClick={() => window.open(deviceCode.verificationUri, '_blank')}
                      className="underline underline-offset-2 font-medium hover:text-indigo-800 dark:hover:text-indigo-200 inline-flex items-center gap-1"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-2xl font-bold tracking-widest text-indigo-800 dark:text-indigo-200 select-all">
                      {deviceCode.userCode}
                    </span>
                    <button
                      onClick={() => navigator.clipboard.writeText(deviceCode.userCode).catch(() => {})}
                      className="p-1.5 rounded-md text-indigo-400 hover:text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors"
                      title="Code kopieren"
                    >
                      <Copy size={14} />
                    </button>
              ...
              {teamsAccounts.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  <span className="text-xs text-zinc-600 dark:text-zinc-400 font-medium">Verbundene Konten</span>
                  <div className="flex flex-col gap-1">
                    {teamsAccounts.map((account) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => teamsLogout(account.id)}
                          className="ml-2 p-1.5 rounded-md text-zinc-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors flex-shrink-0"
              ...
              {/* Konto hinzufügen */}
              <button
                onClick={() => { void teamsLogin() }}
                disabled={!teamsClientId.trim() || !!deviceCode}
                className={[
                  'flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold transition-all',
                  (!teamsClientId.trim() || !!deviceCode)
              ...
              {!teamsClientId.trim() && (
                <p className="text-[11px] text-zinc-400 dark:text-zinc-500 -mt-1">
                  Bitte zuerst die Azure App-ID eingeben.
                </p>
              )}
            </div>
          )}
