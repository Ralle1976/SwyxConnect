...
import { AppSettings, TeamsAccountInfo } from '../types/swyx';
...
interface SettingsStoreState {
  ...
  teamsAccounts: TeamsAccountInfo[];
  ...
  setTeamsClientId: (id: string) => void;
  setTeamsAccounts: (accounts: TeamsAccountInfo[]) => void;
...
export const useSettingsStore = create<SettingsStoreState>()(
  persist(
    (set, get) => ({
      ...
      teamsEnabled: false,
      teamsClientId: '',
      teamsAccounts: [] as TeamsAccountInfo[],
      numberOfLines: 2,
      ...
      setTeamsClientId: (id) => {
        set({ teamsClientId: id });
        get().saveToMain({ teamsClientId: id });
      },
      setTeamsAccounts: (accounts) => {
        set({ teamsAccounts: accounts });
        get().saveToMain({ teamsAccounts: accounts });
      },
        ...
        try {
          ...
          if (settings) {
            set({
              ...
              teamsClientId: settings.teamsClientId ?? '',
              teamsAccounts: settings.teamsAccounts ?? [],
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        teamsClientId: state.teamsClientId,
        teamsAccounts: state.teamsAccounts,
